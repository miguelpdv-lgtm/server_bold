    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:auto;color:#333;">
      ...
      <div style="padding:24px;">
        ...
        <p style="margin-top:16px;font-size:13px;color:#666;">
          ...
          Transaccion Bold: <code>${payment_id}</code>
        </p>
      </div>
  ...
  try {
    ...
    if (payload.type === "SALE_APPROVED") {
      ...
      const { data: pedido, error: fetchError } = await supabase
        .from("orders")
        .select("id, items, estado_pago, r_agendapro, nombre_completo, email, telefono, subtotal, envio, total, direccion, barrio, created_at")
        ...
        if (pedido.email) {
          correosAEnviar.push(
            resend.emails.send({
              ...
              to:      pedido.email,
              subject: `Confirmacion de tu pedido en Emarizos - ${order_id}`,
              html:    buildClienteHTML(pedido, order_id),
            }).then(({ data, error }) => {
        ...
        if (adminEmail) {
          correosAEnviar.push(
            resend.emails.send({
              ...
              html:    buildAdminHTML(pedido, order_id, payment_id),
            }).then(({ data, error }) => {
